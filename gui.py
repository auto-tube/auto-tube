...
class VideoClipperApp:
    ...
    def _show_import_error(self):
        try:
            ...
            ctk.CTkLabel(self.root, text="FATAL ERROR: Failed to load application modules.\nCheck console/logs, project structure, __init__.py files,\nand virtual environment dependencies.",
             ...
             logger.error(f"Failed to display import error in GUI: {ui_err}") # Log UI error too
    ...
    def _show_init_error(self, error):
        try:
             ...
             logger.error(f"Failed to display init error in GUI: {ui_err}") # Log UI error too
    ...
    def _get_config_path(self) -> str:
        ...
        try: # Add try-except around environment variable access
            if system == "Windows": app_support_dir = os.getenv('APPDATA')
                ...
                if app_support_dir and not os.path.isdir(app_support_dir): # Check if path exists
            ...
            # Ensure the base directory exists before joining
            if not os.path.isdir(app_support_dir):
                logger.error(f"Base application support directory does not exist: {app_support_dir}")
                # Handle this error case - perhaps fallback directly?
                raise OSError(f"Base support directory not found: {app_support_dir}")
            ...
            return config_file
        except Exception as e: # Catch potential errors during path manipulation/creation
            logger.error(f"GUI Config Error: Could not determine or create config path: {e}", exc_info=True)
    ...
    def _create_variables(self):
        ...
        # Restore variables needed by UI tabs, even if not used directly in options dict for process_video
        self.remove_audio_var = tk.BooleanVar(value=False)
        self.extract_audio_var = tk.BooleanVar(value=True)
        self.vertical_crop_var = tk.BooleanVar(value=True) # Needed for checkbox even if processing is forced
        self.mirror_var = tk.BooleanVar(value=False)
        self.enhance_var = tk.BooleanVar(value=True)
        # Add variable for codec copy if you have a checkbox/widget for it
        self.use_codec_copy_var = tk.BooleanVar(value=True) # Example - Link this to a widget if needed
    ...
    def _load_settings(self):
        ...
        try:
            ...
            except tk.TclError: logger.warning("Could not display config error messagebox.")
                ...
                # Specific error message will be shown by _check_initial_config
                 ...
                 if VIDEO_PROCESSOR_LOADED:
                      ...
                      try:
                           ...
                           _ = VideoProcessor(output_dir=".", # Corrected arg name - Dummy folder ok
    ...
    def _check_initial_config(self):
        ...
        if AI_UTILS_LOADED:
             ...
             except (ImportError, NameError): # Catch both possible errors
                 logger.error("Could not import or find GEMINI_CONFIGURED flag from ai_utils.")
        else:
        ...
        if TTS_UTILS_LOADED:
             try:
                 ...
                 if not POLLY_CONFIGURED:
                   ...
                   logger.error("Could not import or find POLLY_CONFIGURED flag from tts_utils.")
        else:
        ...
        if warnings:
            ...
            # More robust check to avoid overwriting active status messages
            if "Initializing..." in current_status or "issue(s) detected" in current_status:
        ...
        if UI_TABS_LOADED:
            ...
            logger.info("GUI: All Tab content created.");
             ...
             for tab_name in ["Video Clipper", "AI Short Generator", "Metadata Tools", "Settings"]:
                 try: # Wrap tab access in try-except
                    tab = self.tab_view.tab(tab_name)
                    if tab: # Check if tab was actually created
                         ctk.CTkLabel(tab, text="Error: UI components failed to load.", text_color="red").pack(pady=20)
                 except Exception as tab_err:
                      logger.error(f"Failed to add error label to tab '{tab_name}': {tab_err}")
    ...
    # ***** CORRECTED/VERIFIED THIS METHOD *****
    def _start_clipping_processing(self):
        ...
        try:
            # Read values from Tkinter variables (ensure they exist via _create_variables)
            num_clips_val = self.clip_count_var.get()
            min_duration_val = self.min_clip_length_var.get()
            max_duration_val = self.max_clip_length_var.get()
            use_scene_detect_val = self.scene_detect_var.get()
            scene_threshold_val = self.scene_threshold_var.get()
            # Get value for use_codec_copy (assuming self.use_codec_copy_var exists)
            # Note: VideoProcessor._extract_clip ignores this now because of vertical formatting
            if hasattr(self, 'use_codec_copy_var'):
                 use_codec_copy_val = self.use_codec_copy_var.get()
            else:
                 logger.warning("GUI: 'use_codec_copy_var' not found. Setting 'use_codec_copy' option to False (required for vertical format).")
                 use_codec_copy_val = False # Default to False as re-encode is needed

            # Create the options dictionary with the CORRECT keys expected by process_video
            options = {
                "num_clips": num_clips_val,
                "min_duration": float(min_duration_val), # Ensure float type
                "max_duration": float(max_duration_val), # Ensure float type
                "use_scene_detection": use_scene_detect_val,
                "scene_threshold": scene_threshold_val,
                "use_codec_copy": use_codec_copy_val, # Pass it, but VP will ignore if formatting
                # --- Include options needed ONLY by the manager ---
                "organize_output": self.organize_output_var.get(), # This is used later by run_clipping_queue
            }

            # Basic validation (using the correct keys now)
            assert options["min_duration"] > 0, "Minimum clip duration must be greater than 0."
            assert options["max_duration"] > 0, "Maximum clip duration must be greater than 0."
            assert options["min_duration"] <= options["max_duration"], "Minimum clip duration cannot be greater than Maximum clip duration."
            assert options["num_clips"] > 0, "Number of clips must be greater than 0."
            if options["use_scene_detection"]:
                assert 0.0 < options["scene_threshold"] <= 100.0, "Scene detection threshold must be between 0 (exclusive) and 100 (inclusive)."

            # Log the options *before* starting the thread for debugging
            logger.info(f"GUI: Preparing to start clipping with validated options: {options}")

        except (tk.TclError, ValueError, AssertionError, AttributeError) as e: # Added AttributeError
             logger.error(f"Clipping option validation failed or required variable missing: {e}", exc_info=True) # Log traceback
             messagebox.showerror("Invalid Clipping Option", f"There is an issue with one of the clipping settings:\n\n{e}"); return
        # --- End Get and Validate Options ---
    ...
    def _apply_ai_short_generation(self):
         ...
         ai_options = { # Renamed to avoid conflict with clipping options name
             'polly_voice': polly_voice,
             'font_size': font_size,
             'organize_output': self.organize_output_var.get() # Include organization preference
             # Add other AI short specific options here if needed
             }
         ...
         self.generation_thread = threading.Thread(
             ...
             args=(script_text, video_path, final_output_path, temp_dir_path, ai_options, update_progress_cb, update_status_cb, completion_cb, processing_state),
             daemon=True
         )
    ...
    def _update_progress_bar(self, index, total_items, start_time):
        ...
        if not is_busy: return # Don't update if nothing is supposed to be running

        try:
            if total_items <= 0: self.progress_var.set(0.0); self.remaining_time_var.set("Est. Time: N/A"); return
            current_progress_item = index + 1 # Assume index is 0-based count of completed items/steps
            progress_percent = current_progress_item / total_items
            self.progress_var.set(max(0.0, min(1.0, progress_percent)))
            elapsed_time = time.time() - start_time
            if current_progress_item < total_items and elapsed_time > 1 and current_progress_item > 0:
                time_per_item = elapsed_time / current_progress_item
                remaining_items = total_items - current_progress_item
            ...
            elif current_progress_item >= total_items: self.remaining_time_var.set("Est. Time: Finishing...")
            else: self.remaining_time_var.set("Est. Time: Calculating...")
        except Exception as e:
             logger.error("Error updating progress bar", exc_info=True)
             self.progress_var.set(0.0); self.remaining_time_var.set("Est. Time: Error")
    ...
    def _processing_complete(self, process_type, processed_count, error_count, total_items, processing_state_from_thread):
        ...
        was_stopped = not processing_state_from_thread.get('active', True)

        # --- Reset State ---
        if process_type == "Clipping":
            self.is_processing = False
            if not was_stopped: logger.info("Clipping completed, clearing video queue."); self.video_queue = []; self._update_queue_display(); self.input_path_var.set("")
            else: logger.info("Clipping was stopped, potentially keeping remaining items in queue."); self._update_queue_display();
        ...
        # --- Prepare Messages ---
        msg_func = messagebox.showinfo; msg_title = f"{process_type} Complete"; status_msg = f"Status: Idle. {process_type} finished."; completion_message = ""
        if was_stopped:
            msg_title = f"{process_type} Stopped"; status_msg = f"Status: Idle. {process_type} stopped by user."; completion_message = f"{process_type} process was stopped."; logger.info(f"{process_type} was stopped. Processed before stop: {processed_count}, Errors: {error_count}")
            if process_type == "Clipping" and total_items > 0: completion_message += f"\nItems processed before stop: {processed_count}\nErrors before stop: {error_count}"
        else: # Process completed normally
            completion_message = f"{process_type} finished."; status_msg_end = "successfully."; logger.info(f"{process_type} finished normally. Success: {processed_count}, Errors: {error_count}, Total Attempted: {total_items}")
            if total_items > 0:
                completion_message += f"\nTotal Items Attempted: {total_items}\nSuccessful: {processed_count}\nErrors: {error_count}"
                if process_type == "Clipping":
                    skipped_count = total_items - processed_count - error_count
                    if skipped_count > 0 and (processed_count + error_count) < total_items: completion_message += f"\nSkipped/No Clips: {skipped_count}"
            if error_count > 0:
                status_msg_end = f"with {error_count} error(s)."; msg_func = messagebox.showwarning; msg_title = f"{process_type} Finished with Errors"; completion_message += "\n\nPlease check the application logs for error details."; logger.warning(f"{process_type} finished with {error_count} errors.")
            status_msg = f"Status: Idle. {process_type} {status_msg_end}"

        # --- Update UI ---
        self.status_var.set(status_msg)
        self.progress_var.set(1.0 if not was_stopped and total_items > 0 and processed_count > 0 else 0.0)
        self.remaining_time_var.set("Est. Time: Done")
        # Add try-except around messagebox in case GUI is closing
        try:
            msg_func(msg_title, completion_message)
        except tk.TclError as msg_e:
             logger.warning(f"Could not display completion message box (window closing?): {msg_e}")
    ...
    def _script_generation_complete(self, generated_script: Optional[str], error: Optional[Exception]):
        ...
        if error:
            ...
            if isinstance(error, GeminiError): error_msg = f"Script generation failed (Gemini API Error):\n{error}\n\n(Check API key, quotas, and network connection)"
            else: error_msg = f"Script generation failed unexpectedly ({error_type}):\n{error}"
            logger.error(f"Script Generation Error: {error_msg}", exc_info=isinstance(error, Exception) and not isinstance(error, GeminiError)); self.status_var.set("Status: Idle. Script generation failed!"); messagebox.showerror("Script Generation Error", error_msg[:1000])
        elif generated_script:
            self.status_var.set("Status: Idle. Script generated successfully."); logger.info("GUI: Populating script textbox with generated content.")
            try:
                if not hasattr(self, 'ai_short_tab') or not self.ai_short_tab.winfo_exists() or not hasattr(self.ai_short_tab, 'script_textbox'): raise AttributeError("AI Short Tab or its script textbox UI element not found.")
                script_textbox = self.ai_short_tab.script_textbox; script_textbox.configure(state="normal"); script_textbox.delete("1.0", "end"); script_textbox.insert("1.0", generated_script); messagebox.showinfo("Script Generated", "AI script has been generated and placed in the script text box.")
            except Exception as e:
                 logger.error("Failed to update script textbox UI element", exc_info=True); messagebox.showerror("UI Update Error",f"Failed to display the generated script in the text box:\n{e}\n\nThe script might be printed in the console/log."); print("\n--- GENERATED SCRIPT (UI Update Failed) ---\n" + generated_script + "\n--- END GENERATED SCRIPT ---\n")
        else: logger.warning("Script generation returned successfully but the result was empty."); self.status_var.set("Status: Idle. Generated script was empty."); messagebox.showwarning("Empty Script", "The AI generated an empty script. You might want to try refining your prompt.")
    ...
    def _metadata_generation_complete(self, metadata_type: str, result_list: Optional[List[str]], error: Optional[Exception]):
        ...
        mt = metadata_type.lower(); state_attr = f"is_generating_{mt}"; logger.info(f"GUI Callback: Metadata generation complete for: {mt}")
        if hasattr(self, state_attr): setattr(self, state_attr, False);
        else: logger.warning(f"Could not find state attribute '{state_attr}' to reset.")
        self._update_button_state()
        if error:
            error_type = type(error).__name__
            if isinstance(error, GeminiError): error_msg = f"Failed to generate {mt} (Gemini API Error):\n{error}\n\n(Check API key, quotas, and network connection)"
            else: error_msg = f"Failed to generate {mt} unexpectedly ({error_type}):\n{error}"
            logger.error(f"{mt.capitalize()} Generation Error: {error_msg}", exc_info=isinstance(error, Exception) and not isinstance(error, GeminiError)); self.status_var.set(f"Status: Idle. {mt.capitalize()} generation failed!"); messagebox.showerror(f"{mt.capitalize()} Generation Error", error_msg[:1000])
        elif result_list is not None:
            self.status_var.set(f"Status: Idle. {mt.capitalize()} generated successfully."); logger.info(f"GUI: Populating {mt} output box with {len(result_list)} items.")
            try:
                if not hasattr(self, 'metadata_tab') or not self.metadata_tab.winfo_exists(): raise AttributeError("MetadataTab UI element not found.")
                widget_attr = f"{mt.rstrip('s')}_output_box";
                if not hasattr(self.metadata_tab, widget_attr): raise AttributeError(f"MetadataTab does not have the expected output widget attribute: '{widget_attr}'")
                widget = getattr(self.metadata_tab, widget_attr);
                if not isinstance(widget, ctk.CTkTextbox): raise TypeError(f"Expected widget '{widget_attr}' to be a CTkTextbox, but found {type(widget)}.")
                output_text = "\n".join(result_list); widget.configure(state="normal"); widget.delete("1.0", "end"); widget.insert("1.0", output_text); widget.configure(state="disabled"); logger.info(f"Successfully displayed {len(result_list)} generated {mt}.")
                if not result_list: messagebox.showwarning("Empty Results", f"The AI returned no {mt} based on the provided context.")
            except Exception as e:
                 logger.error(f"Failed to update {mt} output box UI element", exc_info=True); messagebox.showerror("UI Update Error",f"Failed to display the generated {mt} in the output box:\n{e}\n\nResults might be printed in the console/log."); print(f"\n--- GENERATED {mt.upper()} (UI Update Failed) ---\n" + "\n".join(result_list if result_list else []) + f"\n--- END GENERATED {mt.upper()} ---\n")
        else: logger.warning(f"{mt.capitalize()} generation returned None unexpectedly (no error reported)."); self.status_var.set(f"Status: Idle. {mt.capitalize()} generation returned no result."); messagebox.showerror(f"Generation Error", f"An unexpected issue occurred: the AI returned no {mt} results.")
    ...
    def _update_queue_display(self):
        ...
        if hasattr(self,'clipping_tab') and isinstance(self.clipping_tab, ClippingTab) and self.clipping_tab.winfo_exists():
            if hasattr(self.clipping_tab,'update_queue_display') and callable(self.clipping_tab.update_queue_display):
                try: logger.debug("Calling update_queue_display on ClippingTab."); self.clipping_tab.update_queue_display(self.video_queue)
                except Exception as e: logger.error(f"Error calling update_queue_display on ClippingTab: {e}", exc_info=True)
            else: logger.warning("ClippingTab instance exists but is missing the 'update_queue_display' method.")
    ...
    def _update_button_state(self):
        ...
        # logger.debug(f"GUI Helper: Updating button states. Any process busy: {any_busy}") # Less verbose
        ...
        try: # Wrap API flag imports in try-except
            if AI_UTILS_LOADED:
                from utils.ai_utils import GEMINI_CONFIGURED; gemini_ok = GEMINI_CONFIGURED
        except (ImportError, NameError): pass # Defaults to False

        try: # Wrap API flag imports in try-except
            if TTS_UTILS_LOADED:
                from utils.tts_utils import POLLY_CONFIGURED; polly_ok = POLLY_CONFIGURED
        except (ImportError, NameError): pass # Defaults to False

        # logger.debug(f"Button State Checks: FFmpeg OK: {ffmpeg_ok}, Gemini OK: {gemini_ok}, Polly OK: {polly_ok}") # Less verbose

        # Get theme colors (with fallback)
        try:
            default_fg = ctk.ThemeManager.theme["CTkButton"]["fg_color"]; default_hc = ctk.ThemeManager.theme["CTkButton"]["hover_color"]; disabled_fg = ctk.ThemeManager.theme["CTkButton"]["disabled_fg_color"]
        except Exception as theme_err:
            logger.warning(f"Could not get default button colors from ThemeManager: {theme_err}. Using fallback.")
            default_fg = ("#3a7ebf", "#1f538d"); default_hc = ("#325882", "#14375e"); disabled_fg = ("grey50", "grey28")
        ...
        try:
            if hasattr(self, 'clipping_tab') and self.clipping_tab.winfo_exists() and hasattr(self.clipping_tab, 'start_stop_button'):
                ...
                if self.is_processing: btn.configure(text="Stop Clipping", fg_color="red", hover_color="#C40000", state="normal")
                elif not ffmpeg_ok: btn.configure(text="FFmpeg Missing!", state="disabled", fg_color=disabled_fg)
                elif any_busy: btn.configure(text="Start Clipping Queue", state="disabled", fg_color=disabled_fg)
                else: btn.configure(text="Start Clipping Queue", fg_color="green", hover_color="darkgreen", state="normal")
        ...
        try:
            if hasattr(self, 'ai_short_tab') and self.ai_short_tab.winfo_exists() and hasattr(self.ai_short_tab, 'generate_script_button'):
                ...
                if self.is_generating_script: btn.configure(text="Generating Script...", state="disabled", fg_color=disabled_fg)
                elif not gemini_ok: btn.configure(text="Configure Gemini API", state="disabled", fg_color=disabled_fg)
                elif any_busy: btn.configure(text="Generate Script with Gemini", state="disabled", fg_color=disabled_fg)
                else: btn.configure(text="Generate Script with Gemini", state="normal", fg_color=default_fg, hover_color=default_hc)
        ...
        try:
             if hasattr(self, 'ai_short_tab') and self.ai_short_tab.winfo_exists() and hasattr(self.ai_short_tab, 'generate_button'):
                ...
                if self.is_generating_short: btn.configure(text="Generating AI Short...", state="disabled", fg_color=disabled_fg)
                elif not ffmpeg_ok: btn.configure(text="FFmpeg Missing!", state="disabled", fg_color=disabled_fg)
                elif not polly_ok: btn.configure(text="Configure AWS Polly", state="disabled", fg_color=disabled_fg)
                elif any_busy: btn.configure(text="Generate AI Short", state="disabled", fg_color=disabled_fg)
                else: btn.configure(text="Generate AI Short", state="normal", fg_color=default_fg, hover_color=default_hc)
        ...
        try:
            if hasattr(self, 'metadata_tab') and self.metadata_tab.winfo_exists():
                 for meta_type in ['hashtags','tags','titles']:
                     btn_attr = f'generate_{meta_type.rstrip("s")}_button'; busy_attr = f'is_generating_{meta_type}'; btn = getattr(self.metadata_tab, btn_attr, None)
                     if btn and btn.winfo_exists():
                         is_meta_busy = getattr(self, busy_attr, False); button_text_base = f"Generate {meta_type.capitalize()}"
                         if is_meta_busy: btn.configure(text=f"Generating {meta_type.capitalize()}...", state="disabled", fg_color=disabled_fg)
                         elif not gemini_ok: btn.configure(text="Configure Gemini API", state="disabled", fg_color=disabled_fg)
                         elif any_busy: btn.configure(text=button_text_base, state="disabled", fg_color=disabled_fg)
                         else: btn.configure(text=button_text_base, state="normal", fg_color=default_fg, hover_color=default_hc)
    ...
    def _change_theme(self, new_theme: str):
        ...
        logger.info(f"GUI: Changing theme to: {new_theme}"); valid_themes = ["dark", "light", "system"]; theme_lower = new_theme.lower()
        if theme_lower not in valid_themes: logger.warning(f"Invalid theme '{new_theme}' requested. Defaulting to 'system'."); theme_lower = "system"
        ctk.set_appearance_mode(theme_lower); self.theme = theme_lower; logger.info(f"Theme set to '{self.theme}'. Applying to components.")
    ...
    def _apply_treeview_theme_tags(self):
        ...
        if hasattr(self,'clipping_tab') and isinstance(self.clipping_tab, ClippingTab) and self.clipping_tab.winfo_exists():
            if hasattr(self.clipping_tab,'apply_treeview_theme_tags') and callable(self.clipping_tab.apply_treeview_theme_tags):
                try: logger.debug(f"Applying theme '{self.theme}' to Treeview tags."); self.clipping_tab.apply_treeview_theme_tags(self.theme)
                except Exception as e: logger.error(f"Error applying theme tags to Treeview: {e}", exc_info=True)
            else: logger.warning("ClippingTab instance exists but is missing the 'apply_treeview_theme_tags' method.")

    # --- Reset State Methods ---
    def _reset_processing_state(self): logger.debug("Resetting clipping processing state."); self.is_processing = False; self._update_button_state(); self.status_var.set("Status: Idle"); self.progress_var.set(0); self.remaining_time_var.set("Est. Time: N/A")
    def _reset_generation_state(self): logger.debug("Resetting AI short generation state."); self.is_generating_short = False; self._update_button_state(); self.status_var.set("Status: Idle"); self.progress_var.set(0); self.remaining_time_var.set("Est. Time: N/A")
    def _reset_script_gen_state(self): logger.debug("Resetting script generation state."); self.is_generating_script = False; self._update_button_state(); self.status_var.set("Status: Idle")
    ...
    def _reset_metadata_gen_state(self, meta_type: str):
        ...
        if hasattr(self, state_flag_attr):
            ...
            logger.warning(f"Could not find state attribute '{state_flag_attr}' to reset.")
        # Remaining lines of the method
        self._update_button_state()
    ...
    def _on_closing(self):
        ...
        logger.info("GUI: Window close requested."); process_running = (self.is_processing or self.is_generating_short or self.is_generating_script or self.is_generating_hashtags or self.is_generating_tags or self.is_generating_titles); confirm_exit = True
        if process_running: logger.warning("Close requested while a process is running."); confirm_exit = messagebox.askyesno("Confirm Exit", "A background process is still running.\n\nExiting now may cause the process to terminate abruptly and could lead to incomplete or corrupted files.\n\nAre you sure you want to exit?")
        if confirm_exit:
            ...
            # Add signaling for other threads if implemented
            # Consider joining threads with a timeout if robust cleanup is needed
            self.root.destroy()
        else: logger.info("GUI: Exit cancelled by user.")
...
def main():
    ...
    try: # Line ~1150
        ...
        try: # Line ~1156 - This try handles the messagebox display
            import tkinter as tk; from tkinter import messagebox; error_root = tk.Tk(); error_root.withdraw(); messagebox.showerror("Fatal Startup Error", f"Failed to initialize the main application window.\n\nError: {e}\n\nPlease check logs and ensure TkinterDnD2 is installed correctly."); error_root.destroy()
        except Exception as fallback_e:
             logger.error(f"Could not display Tkinter error message: {fallback_e}")
             print(f"FATAL STARTUP ERROR: {e}")
        sys.exit(1)
    # Check if root was successfully created before proceeding
    if root:
        if MODULE_IMPORTS_OK:
            try:
                ...
                # Use try-except for messagebox in case Tkinter itself is broken
                try:
                    messagebox.showerror("Fatal Application Error", f"A critical error occurred during application runtime:\n\n{app_e}\n\nPlease check logs for details.")
                except Exception:
                    print(f"FATAL APP ERROR (messagebox failed): {app_e}")
                try:
    ...
    else: # Line ~1198 - This else corresponds to 'if root:'
...
if __name__ == "__main__":
    # This try block starts around line 1190
    try:
        logger.info(f"--- {APP_NAME} Application Start ---")
        logger.info(f"Python Version: {sys.version.split()[0]}")
        logger.info(f"Platform: {platform.system()} ({platform.release()})")
        logger.info(f"CustomTkinter Version: {ctk.__version__}")
    except NameError: # Around Line 1196
        print("Logger not initialized before __main__ guard."); sys.exit(1)
    except Exception as init_log_e: # Line 1198
         logger.error(f"Error during initial info logging: {init_log_e}") # Line 1199

    main() # Line 1201

    logger.info(f"--- {APP_NAME} Application End ---") # Line 1203